const basicType={
    ...
    opacity:{
        ...
        setValue:(item,value)=>item.opacity= +value//转换为数字
    },
    transparent:{
    ...
    side:{
        extends:[['frond','back','double']],
        getValue:(item,camera)=>'frond',
...
function createMaterial(geometry){
    ...
    const mesh =new THREE.SceneUtils.createMultiMaterialObject(geometry,[
        lambert,
        basic
    ])
    return mesh
...
function getMeshValue(extend,name,scene){
    return {
        extends:extend,
        getValue:(item,camera,mesh)=>isFont(item)&&textOptions[name]!==undefined?textOptions[name]: mesh.pointer.children[0].geometry.parameters[name],
        setValue:(...args)=>removeAndAdd(...args)
    }
}
...
const itemType={
    ...
    MeshDepthMaterial:['wireframe','cameraNear','cameraFar'],//基础材质
    ...
    CylinderGeometry:['radiusTop','radiusBottom','height','heightSegments','radialSegments','openEnded'],
    ...
    TextGeometry:['size','font','bevelThickness','bevelSize','bevelEnabled','bevelSegments','curveSegments','steps'],//
...
function initControls(item,camera,mesh,scene){
    ...
    typeList.forEach(items=>{
        const child=basicType[items];
        if(child){
            controls[items]=child.getValue(item,camera,mesh);
            const childExtends=child.extends||[];
            gui[child.method||'add'](controls,items,...childExtends).onChange(value=>{
